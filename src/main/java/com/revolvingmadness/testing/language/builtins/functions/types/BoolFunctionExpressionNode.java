...
public class BoolFunctionExpressionNode implements LiteralExpressionNode {
    @Override
    public LiteralExpressionNode call(ScriptNode script, List<ExpressionNode> arguments) {
        if (arguments.size() != 1) {
            throw new SyntaxError("Function 'bool' requires 1 argument but got " + arguments.size() + " argument(s)");
        }
