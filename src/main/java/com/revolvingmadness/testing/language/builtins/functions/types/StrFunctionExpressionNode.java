...
public class StrFunctionExpressionNode implements LiteralExpressionNode {
    @Override
    public LiteralExpressionNode call(ScriptNode script, List<ExpressionNode> arguments) {
        if (arguments.size() != 1) {
            throw new SyntaxError("Function 'str' requires 1 argument but got " + arguments.size() + " argument(s)");
        }
